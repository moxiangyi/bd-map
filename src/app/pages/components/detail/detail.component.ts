...
export class DetailComponent implements OnInit {
  ...
  constructor(
    ...
    this.pointDataInfo = JSON.parse(localStorage.getItem('pointDataInfo')) || this.databaseService.pointDataInfo;
    if (this.id) {
      this.filterItem = this.pointDataInfo.find(item => {
  ...
  save() {
    if (this.id) {
      ...
      localStorage.setItem('pointDataInfo', JSON.stringify(this.pointDataInfo));
    } else {
      const id = this.pointDataInfo[this.pointDataInfo.length - 1].id;
      ...
      localStorage.setItem('pointDataInfo', JSON.stringify(this.databaseService.pointDataInfo));
    }
    this.cancel();
  }
